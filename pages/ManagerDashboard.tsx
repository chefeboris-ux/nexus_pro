...
const loadAllSales = async () => {
  // Carregar todas as Vendas do Banco de Dados
...
useEffect(() => {
  loadAllSales();
  loadAllSales();
  const interval = setInterval(loadAllSales, 10000); // 10s refresh
  return () => {
...
const handleUpdateStatus = (id: string, newStatus: SaleStatus, reason?: string) => {
  ...
  const updated = await SalesService.updateStatus(id, newStatus, newHistoryEntry, (isReturn || isRegression) ? reason : undefined);
